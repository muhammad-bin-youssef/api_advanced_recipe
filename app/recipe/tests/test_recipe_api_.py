...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_create_ingredient_on_update(self):
        ...
        ingre = Ingredient.objects.get(user=self.user, name="Fish Sauce")
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(ingre, recipe.ingredients.all())

    def test_update_recipe_assign_ingredient(self):
        old_ingredient = Ingredient.objects.create(user=self.user, name="Chili")
        recipe = create_recipe(user=self.user)
        recipe.ingredients.add(old_ingredient)
        new_ingredient = Ingredient.objects.create(user=self.user, name="Sugar")
...
class ImageUplooadTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = create_user(email="user@example.com", password="test123")
        self.client.force_authenticate(self.user)
